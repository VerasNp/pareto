...
class FloodFill {
	...
	public fill(img: Image, x: number, y: number, oldColor: Color, newColor: Color): void {
		const pixel = img.getPixel(x, y)
		if (!pixel) return
		img.setPixel(x, y, newColor)
		if (x > 0 && img.getPixel(x - 1, y)?.equals(oldColor)) {
			this.fill(img, x - 1, y, oldColor, newColor)
		} else if (x < img.getScreenWidth() - 1 && img.getPixel(x + 1, y)?.equals(oldColor)) {
			this.fill(img, x + 1, y, oldColor, newColor)
		} else if (y > 0 && img.getPixel(x, y - 1)?.equals(oldColor)) {
			this.fill(img, x, y - 1, oldColor, newColor)
		} else if (y < img.getScreenHeight() - 1 && img.getPixel(x, y + 1)?.equals(oldColor)) {
			this.fill(img, x, y + 1, oldColor, newColor)
		}
	}
}
